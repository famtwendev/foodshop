package com.famtwen.gateway.configuration;

import com.famtwen.gateway.dtos.ApiResponse;
import com.famtwen.gateway.services.IdentityService;
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        return identityService.introspect(token)
                              .flatMap(introspectResponse -> {
                                  log.info("Introspect raw response: {}", introspectResponse);
                                  if (introspectResponse.getResult()
                                                        .isActive())
                                      return chain.filter(exchange);
                                  else
                                      return unauthenticated(exchange.getResponse());
                              ...
                              .onErrorResume(throwable -> {
                                  log.error("Error during introspect", throwable);
                                  return unauthenticated(exchange.getResponse());
                              });
    }
